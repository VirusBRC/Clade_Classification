...
sub _getUniqueAccession {
  my ncbi::HxNyClassifier $this = shift;
...
sub _readProfileClade {
  ...
  my $lookup = LOOKUP_TYPE;
  if (
    !util::Constants::EMPTY_LINE( $properties->{profileSeqs}->{lookupType} ) )
  {
    $lookup = join( util::Constants::DOT,
      $properties->{profileSeqs}->{lookupType}, LOOKUP_TYPE );
  }
  my $fh = $utils->openFile( $this->{profile}->{$lookup}, '<' );
  $this->{profile_clade} = {};
  while ( !$fh->eof ) {
    my $line = $fh->getline;
    chomp($line);
...
sub _checkErrorStatus {
  ...
  $this->{jobs}->terminateRun( 'hxny', keys %{ $this->{pids} } );
}
...
sub _updateResult {
  ...
  while ( !$fh->eof ) {
    ...
    chomp($line);
    next if ( util::Constants::EMPTY_LINE($line) );
    my ( $acc, $clade, $tree ) = split( /\t/, $line );
    next if ( $acc ne $gbAccession );
    $this->postProcessClade( $struct, $clade );
  }
  $fh->close;
}
...
sub _launchNextJob {
  ...
  $this->{error_mgr}->printMsg("Creating process $acc");
...
sub new {
  ...
  $this->{dataset_name} = $ncbi_utils->getProperties->{datasetName};
...
sub generateLoad {
  ...
  if ( scalar @{ $properties->{naSequenceIds} } == 0 ) {
    $ncbi_utils->addReport(
      "$datasetName classification annotation:  no sequences found\n");
    return;
  }
  ...
  $this->{counts} =
    new util::Statistics( "$datasetName Classification Sequence Disposition",
  ...
  foreach my $struct (@data) {
    ...
    $struct->{hxny_clade} = util::Constants::EMPTY_STR;
    ###
    ### check for profile sequence
    ###
    if ( defined( $profile_clade->{$acc} ) ) {
      $struct->{hxny_clade} = $profile_clade->{$acc};
      $this->{error_mgr}
...
sub processToDb {
  ...
  my $loader = new ncbi::Loader( "$datasetName annotation",
...
OUTTER_LOOP:
  ...
  if ($queryStatus) {
    $ncbi_utils->addReport(
"$datasetName classification annotation (ERRORS ENCOUNTERED):  inserts executed "
  ...
  ### Successful Run
  ###
  $ncbi_utils->addReport( "$datasetName classification annotation:  success "
      . ( $db->getCommitCount - $errorCount )
      . ", error $errorCount\n" );
...
=head2 B<new ncbi::HxNyClassifier(error_mgr, tools, utils, ncbi_utils)>
